"""Estimate the Green's function for first-step perturbations of ``m1``."""
...
app = typer.Typer(
    ...
    help=(
        "Compute the Green's function for an impulsive perturbation of m1. "
...
def run_signed_response(job: ResponseJob) -> ResponseResult:
    """Run one response trajectory with a first-step perturbation of ``m1``."""
    ...
    perturbed_params = replace(
        job.params,
        m1=job.params.m1 + job.sign * job.epsilon / job.stochastic_settings.dt,
    )
...
def response_dataset(
    *,
    ...
    attrs: dict[str, object] = {
        "title": "Green's function for first-step m1 perturbations",
        "latitude_name": "normalized latitude",
        "latitude_bounds": "[-1, 1]",
        ...
        "m1_plus_first_step": float(params.m1 + epsilon / stochastic_settings.dt),
        "m1_minus_first_step": float(params.m1 - epsilon / stochastic_settings.dt),
        "perturbation_note": "m1 is perturbed only during the first IMEX step.",
        ...
        "green_function_units_note": (
            ...
            "is m1 +/- epsilon / dt. The Green's function is K per unit m1-impulse "
            "(m1 * s); m1 is dimensionless so this reduces to K s^-1."
        ),
    }
...
def main(
    ...
    filename: str = typer.Option("response_co2.nc", help="Output filename for the NetCDF response dataset."),
    output_dir: str | None = typer.Option(
        None,
        help="Directory in which to write the NetCDF file; defaults to the repository data directory.",
    ),
    mu: float = typer.Option(1.0, help="Relative solar strength parameter used when generating samples."),
    ...
    epsilon: float = typer.Option(0.015, help="Impulse amplitude used in m1 +/- epsilon / dt."),
    ...
    """Compute and save the m1 response Green's function."""
    ...
    log(
        (
            f"first-step perturbations: m1+={params.m1 + epsilon / stochastic_settings.dt:.12g}, "
            f"m1-={params.m1 - epsilon / stochastic_settings.dt:.12g}"
        )
    )
